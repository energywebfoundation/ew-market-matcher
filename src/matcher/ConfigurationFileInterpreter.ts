...
const simpleRanking = (rule: RuleConf.SimpleHierarchyRule): string[] => {
  ...
  propertyNames.forEach(
    (name, index) => (debugOutput += index + 1 + ". " + name + "  ")
  );
  ...
  switch (simpleHierarchyRelevantProperty.mappingFormat) {
    ...
    case RuleConf.MappingFormat.Defined:
      ...
      const mapping = (simpleHierarchyRelevantProperty as RuleConf.MappedSimpleHierarchyRelevantProperty).mapping.find(
        (item: RuleConf.SimpleHierarchyRelecantPropertyMapping) =>
          item.valueToMap === value
      );

      return mapping
        ? mapping.mappedValue
        : getDefaultValue(simpleHierarchyRelevantProperty);

    default:
      throw new Error(
        "Unknown mapping format " +
          simpleHierarchyRelevantProperty.mappingFormat
      );
  }
};
...
const getDefaultValue = (
  ...
  switch (simpleHierarchyRelevantProperty.defaultValue.type) {
    ...
    default:
      throw new Error(
        "Unknown default value type " +
          simpleHierarchyRelevantProperty.defaultValue.type
      );
  }
};
