...
export class BlockchainModeController extends Controller {
  ...
  async registerAgreement(newAggreement: EwMarket.Agreement.Entity) {
    ...
    if (allowed) {
      ...
      logger.verbose(
        "This instance is not an machter for agreement #" + newAggreement.id
      );
    }
  }
  ...
  async matchDemand(
    ...
    logger.info(
      "Matched certificate #" + certificate.id + " to demand #" + demand.id
    );
  }
  ...
  async getCurrentPeriod(
    ...
    this.date = (await this.conf.blockchainProperties.web3.eth.getBlock(
      "latest"
    )).timestamp;
    switch (timeFrame) {
