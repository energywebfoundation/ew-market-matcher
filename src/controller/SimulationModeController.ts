...
export class SimulationModeController extends Controller {
  ...
  constructor(simulationDataSource: SimulationDataSource) {
    ...
    logger.verbose(
      "Loaded simulation flow file containing " +
        this.simulationFlow.flow.length +
        " actions"
    );
      ...
      if (
        !this.agreements.find(
          (aggreement: EwMarket.Agreement.Entity) =>
            newAggreement.id === aggreement.id
        )
      ) {
  ...
  getDemand(demandId: string): EwMarket.Demand.Entity {
    return this.demands.find(
      (demand: EwMarket.Demand.Entity) => demand.id === demandId
    );
  }

  getSupply(supplyId: string): EwMarket.Supply.Entity {
    return this.supplies.find(
      (supply: EwMarket.Supply.Entity) => supply.id === supplyId
    );
  }
  ...
  setDataSourceTime(dateData: SimulationFlowDef.Date.DateData) {
    ...
    logger.verbose(
      "Set simulation time to " + new Date(this.date * 1000).toUTCString()
    );
  }
    ...
    if (expectedMatch[key] !== match[key]) {
      logger.debug(
        `Match #${expectedMatchIndex}: ${key} (${
          match[key]
        }) does not equal expected ${key} (${expectedMatch[key]}) ${
          LogSymbols.error
        }`
      );
  ...
  async splitCertificate(
    ...
    logger.debug(
      "First child #" + firstChild.id + " with " + firstChild.powerInW + "wh"
    );
    logger.debug(
      "Second child #" + secondChild.id + " with " + secondChild.powerInW + "wh"
    );
  ...
  compareWithExpectedResults() {
    ...
    this.matches.forEach((match: SimulationFlowDef.Match, index: number) => {
      if (this.simulationFlow.expectedResult[index]) {
        ...
        if (matchFits) {
          logger.verbose(
            "Match #" + index + " as expected " + LogSymbols.success
          );
        } else {
    ...
    if (expectedResultFits) {
      logger.info(
        "Complete match between expected results and actual results " +
          LogSymbols.success
      );
    } else {
      logger.info(
        "At least one result does not macht an expected result " +
          LogSymbols.error
      );
    }
  }
