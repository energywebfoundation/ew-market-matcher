...
import {
  BlockchainDataSourceType,
  BlockchainDataSource
} from "../schema-defs/MatcherConf";
...
export const initMatchingManager = async (
  ...
  const agreementListLength = await EwMarket.Agreement.getAgreementListLength(
    conf
  );
  ...
  for (let i = 0; i < demandListLength; i++) {
    controller.registerDemand(
      await new EwMarket.Demand.Entity(i.toString(), conf).sync()
    );
  }
  ...
  for (let i = 0; i < supplyListLength; i++) {
    controller.registerSupply(
      await new EwMarket.Supply.Entity(i.toString(), conf).sync()
    );
  }

  conf.logger.verbose("* Getting all certificates");
  const certificateListLength = await EwOrigin.Certificate.getCertificateListLength(
    conf
  );
...
export const initEventHandling = async (
  ...
  certificateContractEventHandler.onEvent(
    "LogCreatedCertificate",
    async (event: any) => {
      logger.verbose(
        "Event: LogCreatedCertificate certificate #" +
  ...
  assetContractEventHandler.onEvent("LogAssetFullyInitialized", async event => {
    console.log(
      "\n* Event: LogAssetFullyInitialized asset: " +
  ...
  consumingAssetContractEventHandler.onEvent(
    "LogAssetFullyInitialized",
    async event => {
      console.log(
        "\n* Event: LogAssetFullyInitialized consuming asset: " +
          event.returnValues._assetId
      );
  ...
  consumingAssetContractEventHandler.onEvent(
    ...
    async event => {
      console.log(
        "\n* Event: LogAssetSetActive consuming asset: " +
          event.returnValues._assetId
      );
  ...
  consumingAssetContractEventHandler.onEvent(
    ...
    async event => {
      console.log(
        "\n* Event: LogAssetSetInactive consuming asset: " +
          event.returnValues._assetId
      );
